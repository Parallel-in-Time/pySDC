...
def test_canintegratepolynomials():
    for collclass in classes:
      for M in range(2,13):
          yield check_canintegratepolynomials, collclass, M, t_start, t_end

def check_canintegratepolynomials(collclass,M,t_start,t_end):
...
def test_relateQandSmat():
    for collclass in classes:
      for M in range(2,13):
          yield check_relateQandSmat, collclass, M, t_start, t_end


def check_relateQandSmat(collclass,M,t_start,t_end):
    coll = collclass(M, t_start, t_end)
    Q = coll.Qmat[1:,1:]
    S = coll.Smat[1:,1:]
...
def test_partialquadrature():
    for collclass in classes:
        for M in range(2, 13):
            yield check_partialquadraturewithQ, collclass, M, t_start, t_end
...
def test_partialquadraturewithS():
    for collclass in classes:
        for M in range(2, 13):
            yield check_partialquadraturewithS, collclass, M, t_start, t_end
