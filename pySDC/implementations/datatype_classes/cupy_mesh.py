...
class cupy_mesh(cp.ndarray):
    ...
    def __new__(cls, init, val=0.0, offset=0, buffer=None, strides=None, order=None):
        ...
        if isinstance(init, cupy_mesh):
            obj = cp.ndarray.__new__(cls, shape=init.shape, dtype=init.dtype, strides=strides, order=order) # TODO: not in cp.ndarray , buffer=buffer, offset=offset
            obj[:] = init[:]
            ...
            # obj.fill(val)
            obj.full(shape=init[0], fill_value=val, dtype=init[2], order=order)
        else:
