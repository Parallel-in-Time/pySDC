...
class mesh_to_mesh_fft2d(space_transfer):
    ...
    def restrict(self, F):
        ...
        if isinstance(F, cupy_mesh):
            G = cupy_mesh(self.coarse_prob.init, val=0.0)
            G[:] = F[::self.ratio, ::self.ratio]
        elif isinstance(F, imex_cupy_mesh):
            G = imex_cupy_mesh(self.coarse_prob.init, val=0.0)
    ...
    def prolong(self, G):
        ...
        if isinstance(G, cupy_mesh):
            F = cupy_mesh(self.fine_prob.init)
            tmpG = cp.fft.fft2(G)
            tmpF = cp.zeros(self.fine_prob.init[0], dtype=cp.complex128)
            ...
            F[:] = cp.real(cp.fft.ifft2(tmpF)) * self.ratio * 2
        elif isinstance(G, imex_cupy_mesh):
            F = imex_cupy_mesh(G)
            tmpG_impl = cp.fft.fft2(G.impl)
            tmpF_impl = cp.zeros(self.fine_prob.init, dtype=cp.complex128)
            halfG = int(self.coarse_prob.init[0][0] / 2)
            ...
            F.impl[:] = cp.real(cp.fft.ifft2(tmpF_impl)) * self.ratio * 2
            tmpG_expl = cp.fft.fft2(G.expl) / (self.coarse_prob.init[0] * self.coarse_prob.init[1])
            tmpF_expl = cp.zeros(self.fine_prob.init[0], dtype=cp.complex128)
            halfG = int(self.coarse_prob.init[0][0] / 2)
            ...
            F.expl[:] = cp.real(cp.fft.ifft2(tmpF_expl)) * self.ratio * 2
        else:
