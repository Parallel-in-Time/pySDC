...
class fenics_heat_mass_timebc_lift(fenics_heat_mass_timebc):
    r"""
    Example implementing the forced one-dimensional heat equation with time-dependent Dirichlet boundary conditions
    using boundary lifting to restore full order of convergence.
    ...
    where :math:`E(x, t)` is a lift function satisfying the time-dependent boundary conditions, and
    :math:`v(x, t)` satisfies homogeneous Dirichlet boundary conditions. The lift is chosen as the
    linear interpolation of the boundary data:

    .. math::
        E(x, t) = (1 - 2x)\cos(t) + c.

    The equation for the transformed variable :math:`v = u - E` becomes

    .. math::
        \frac{d v}{d t} = \nu \frac{d^2 v}{d x^2} + \tilde{f},

    where the modified forcing term accounts for the lift:

    .. math::
        \tilde{f}(x, t) = f(x, t) - \frac{\partial E}{\partial t} + \nu \Delta E
            = -\cos(\pi x)(\sin(t) - \nu \pi^2 \cos(t)) + (1 - 2x)\sin(t).

    Since :math:`E` is linear in :math:`x`, we have :math:`\Delta E = 0`, and
    :math:`\partial E / \partial t = -(1-2x)\sin(t)`.

    The exact solution of the transformed problem is

    .. math::
        v(x, t) = \cos(t)(\cos(\pi x) - 1 + 2x).

    This approach restores the full order of convergence that may be lost when time-dependent
    boundary conditions are applied directly, as described in [3]_.
    ...
    family : str, optional
        Indicates the family of elements used to create the function space
        for the trial and test functions. The default is ``'CG'``, which are the class
        of Continuous Galerkin, a *synonym* for the Lagrange family of elements, see [2]_.
    order : int, optional
        Defines the order of the elements in the function space.
    refinements : int, optional
    ...
    nu : float, optional
        Diffusion coefficient :math:`\nu`.
    c : float, optional
        Constant used in the lift :math:`E` (cancels in the transformed variable :math:`v`).
    ...
    K : scalar, vector, matrix or higher rank tensor
        Denotes the expression :math:`- \nu \int_\Omega \nabla u \nabla v\,dx`.
    g : Expression
        The modified forcing term :math:`\tilde{f}` including the lift correction.
    bc_hom : DirichletBC
        Denotes the homogeneous Dirichlet boundary conditions for the transformed variable.
    ...
    .. [2] Automated Solution of Differential Equations by the Finite Element Method. A. Logg, K.-A. Mardal, G. N.
        Wells and others. Springer (2012).
    .. [3] Spectral Deferred Correction Methods for Ordinary Differential Equations. A. Dutt, L. Greengard, V. Rokhlin.
        Mathematics of Computation, 2000. https://dl.acm.org/doi/abs/10.1090/S0025-5718-01-01362-X
    """
    ...
    def __init__(self, c_nvars=128, t0=0.0, family='CG', order=4, refinements=1, nu=0.1, c=0.0):
        ...
        #   dE/dt    = -(1 - 2*x) * sin(t)
        #   Delta E  = 0  (E is linear in x, so second spatial derivative vanishes)
        # Modified forcing: f_tilde = f - dE/dt + nu * Delta E
    ...
    def solve_system(self, rhs, factor, u0, t):
        ...
        Uses homogeneous Dirichlet boundary conditions since the transformed variable :math:`v = u - E`
        satisfies :math:`v = 0` on the boundary.
