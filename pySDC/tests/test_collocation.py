...
def test_canintegratepolynomials(collclass):
    ...
    for M in range(2,13):
        ...
        assert abs(int_ex - int_coll) < 1e-11, "For node type " + coll.__class__.__name__ + ", failed to integrate polynomial of degree " + str(coll.order-1) + " exactly. Error: %5.3e" % abs(int_ex - int_coll)
...
def test_partialquadraturewithQ(collclass):
    for M in range(2, 13):
        ...
        for i in range(0,M):
            ...
            assert abs(int_ex - int_coll)< 2e-11, "For node type " + coll.__class__.__name__ + ", partial quadrature from Qmat rule failed to integrate polynomial of degree M-1 exactly for M = " + str(M)
...
def test_partialquadraturewithS(collclass):
    for M in range(2, 13):
        ...
        for i in range(1,M):
            ...
            assert abs(int_ex - int_coll)<2e-11, "For node type " + coll.__class__.__name__ + ", partial quadrature rule from Smat failed to integrate polynomial of degree M-1 exactly for M = " + str(M)
