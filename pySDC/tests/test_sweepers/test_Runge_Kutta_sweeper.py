...
def single_run(sweeper_name, dt, lambdas, use_RK_sweeper=True, Tend=None, useGPU=False):
    ...
    Args:
        sweeper_name (str): Name of Runge-Kutta method
...
def test_order(sweeper_name, useGPU=False):
    """
    ...
    for dt in dts:
        stats, _, controller = single_run(sweeper_name, dt, lambdas, useGPU=useGPU)
...
def test_stability(sweeper_name, useGPU=False):
    """
    ...
    stats, ic, _ = single_run(sweeper_name, 1.0, lambdas.copy(), Tend=1.0, useGPU=useGPU)
...
@pytest.mark.base
@pytest.mark.parametrize("sweeper_name", SWEEPER_NAMES + IMEX_SWEEPERS)
def test_rhs_evals(sweeper_name, useGPU=False):
    """
...
def test_sweeper_equivalence(sweeper_name, useGPU=False):
    from pySDC.helpers.stats_helper import get_sorted
    import numpy as np
    ...
    for use_RK_sweeper in [True, False]:
        (
            ...
            _,
            _,
        ) = single_run(sweeper_name, 1.0, [[-1 + 0j]], use_RK_sweeper=use_RK_sweeper, Tend=2.0, useGPU=useGPU)
...
@pytest.mark.cupy
@pytest.mark.parametrize("test_name", ["test_rhs_evals", 'test_stability', 'test_order'])
@pytest.mark.parametrize("sweeper_name", SWEEPER_NAMES)
def test_RK_sweepers_with_GPU(test_name, sweeper_name):
    if test_name == 'test_rhs_evals':
        test_rhs_evals(sweeper_name=sweeper_name, useGPU=True)
    elif test_name == 'test_stability':
        test_stability(sweeper_name=sweeper_name, useGPU=True)
    elif test_name == 'test_order':
        test_order(sweeper_name=sweeper_name, useGPU=True)
