...
def test_monodomain_convergence_ESDC_TTP_one_level():
...
@pytest.mark.monodomain
def test_monodomain_convergence_ESDC_TTP_two_levels():
    from pySDC.projects.Monodomain.run_scripts.run_MonodomainODE import setup_and_run
...
@pytest.mark.monodomain
def test_monodomain_convergence_SDC_HH_one_level():
    from pySDC.projects.Monodomain.run_scripts.run_MonodomainODE import setup_and_run
    ...
    refinements = [0]
    order = 2  # 2 or 4
    ionic_model_name = "HH"  # HH is already smooth
    enable_output = False
    ...
    # since we use standard SDC here, with 3 nodes the underlying method is RADAU5 and we should get order 5
    assert np.all(rates > 4.5), "ERROR: convergence rate is too low!"
    assert np.all(rates < 5.5), "ERROR: convergence rate is too high!"


# if __name__ == "__main__":
# test_monodomain_convergence_ESDC_TTP_one_level()
# test_monodomain_convergence_ESDC_TTP_two_levels()
# test_monodomain_convergence_SDC_HH_one_level()
