...
def next_neighbors_periodic(p, ps, k):
    ...
    """
    p_bar = p - cp.floor(p / 1.0) * 1.0
    ps = ps - ps[0]
    distance_to_p = cp.asarray(
        list(map(lambda tk: min([cp.abs(tk + 1 - p_bar), cp.abs(tk - p_bar), cp.abs(tk - 1 - p_bar)]), ps)))
...
def continue_periodic_array(arr, nn):
    ...
    nn = cp.asarray(nn)
    d_nn = nn[1:] - nn[:-1]
    if cp.all(d_nn == cp.ones(nn.shape[0] - 1)):
        return arr[nn]
    else:
        cont_arr = [arr[nn[0]]]
        shift = 0.
...
def restriction_matrix_1d(fine_grid, coarse_grid, k=2, periodic=False, pad=1):
    ...
    if periodic:
        M = cp.zeros((coarse_grid.size, fine_grid.size))
        for i, p in zip(range(n_g), coarse_grid):
            ...
            if p > cp.mean(coarse_grid) and not (cont_arr[0] <= p <= cont_arr[-1]):
                cont_arr += 1
            bary_pol = []
            for l in range(k):
                bary_pol.append(BarycentricInterpolator(cont_arr.get(), np.roll(circulating_one, l)))
            M[i, nn] = cp.asarray(list(map(lambda x: x(p), bary_pol)))
    else:
        M = cp.zeros((coarse_grid.size, fine_grid.size + 2 * pad))
        for i, p in zip(range(n_g), coarse_grid):
            ...
            bary_pol = []
            for l in range(k):
                bary_pol.append(BarycentricInterpolator(padded_f_grid[nn].get(), np.roll(circulating_one, l)))
            M[i, nn] = cp.asarray(list(map(lambda x: x(p), bary_pol)))
        if pad > 0:
            M = M[:, pad:-pad]

    return csp.csc_matrix(M)
...
def interpolation_matrix_1d(fine_grid, coarse_grid, k=2, periodic=False, pad=1, equidist_nested=True):
    ...
    if periodic:
        ...
        M = cp.zeros((fine_grid.size, coarse_grid.size))
        ...
        if equidist_nested:
            ...
            for i, p in zip(range(n_f), fine_grid):
                ...
                if i % 2 == 0:
                    ...
                    if p > cp.mean(fine_grid) and not (cont_arr[0] <= p <= cont_arr[-1]):
                        cont_arr += 1
        ...
        M = cp.zeros((fine_grid.size, coarse_grid.size + 2 * pad))
        ...
        if equidist_nested:
            ...
            for i, p in zip(range(n_f), fine_grid):
                ...
                if i % 2 != 0:
                    ...
                    bary_pol = []
                    for l in range(k):
                        bary_pol.append(BarycentricInterpolator(padded_c_grid[nn].get(), np.roll(circulating_one, l)))
                    M[i, nn] = cp.asarray(list(map(lambda x: x(p), bary_pol)))
...
def border_padding(grid, l, r, pad_type='mirror'):
    ...
    padded_arr = cp.zeros(grid.size + l + r)
    if pad_type == 'mirror':
        for i in range(l):
