...
from pySDC.implementations.transfer_classes.TransferMesh_NoCoarse import (
    mesh_to_mesh as mesh_to_mesh_nc,
)
...
# from pySDC.implementations.hook

from pySDC.projects.compression.sweeper_imex_compression import (
    imex_1st_order_compression,
)
...
def setup_diffusion(dt=None, ndim=None, ml=False):
    ...
    if ml:
        problem_params["nvars"] = [
            tuple(64 for _ in range(ndim)),
            tuple(32 for _ in range(ndim)),
        ]  # number of dofs
    else:
    ...
    description["sweeper_class"] = imex_1st_order_compression  # pass sweeper (see part B)
