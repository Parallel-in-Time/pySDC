...
from pySDC.implementations.problem_classes.Battery_extended import battery_extended
...
def main(use_switch_estimator=True, use_adaptivity=False):
    ...
    description = dict()
    description['problem_class'] = battery_extended  # pass problem class
    ...
    # fname = 'data/battery_extended.dat'
    fname = 'battery_extended.dat'
    f = open(fname, 'wb')
    dill.dump(stats, f)
    f.close()
    ...
    f = open('battery_extended_out.txt', 'w')
...
def plot_voltages(description, use_switch_estimator, use_adaptivity, cwd='./'):
    ...
    f = open(cwd + 'battery_extended.dat', 'rb')
    stats = dill.load(f)
    f.close()
