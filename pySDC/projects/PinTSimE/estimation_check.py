...
from pySDC.projects.PinTSimE.battery_model import run_simulation, plot_styling_stuff
...
def run_estimation_check():
    ...
    for problem, sweeper, prob_cls_name in zip(problem_classes, sweeper_classes, prob_class_names):
        u_num = run_simulation(
            problem=problem,
            problem_params=problem_params[prob_cls_name],
            sweeper=sweeper,
        ...
        accuracy_check(u_num, prob_cls_name)

        state_function_around_event(u_num, prob_cls_name)

        state_function_over_time(u_num, prob_cls_name)


def accuracy_check(u_num, prob_cls_name):  # pragma: no cover
    """
    ...
    for dt in dt_list:
        ...
        e_ax = ax.twinx()
        for use_SE in u_num[dt].keys():
            dt_val = u_num[dt][use_SE][use_A]['dt']
            e_em_val = u_num[dt][use_SE][use_A]['e_em']
            if use_SE: 
                t_switches = u_num[dt][use_SE][use_A]['t_switches']
        ...
        fig.savefig('data/detection_and_adaptivity_{}_dt={}.png'.format(prob_cls_name, dt), dpi=300, bbox_inches='tight')
        plt_helper.plt.close(fig)    


def state_function_around_event(u_num, prob_cls_name):  # pragma: no cover
    r"""
    ...
    ----------
    u_num : dict
        Contains all the data sorted in appropriate manner. Details can be found in the routine
        ``get_data_dict()`` from ``pySDC.projects.PinTSimE.battery_model``.
    prob_cls_name : str
        Name of the problem class.
    """
    ...
    use_detection = u_num[list(dt_list)[0]].keys()
    use_adaptivity = u_num[list(dt_list)[0]][list(use_detection)[0]].keys()
    h0 = u_num[list(dt_list)[0]][list(use_detection)[0]][list(use_adaptivity)[0]]['state_function']
    n = h0[0].shape[0]
    ...
    for i in range(n):
        ...
        for use_SE in use_detection:
            for use_A in use_adaptivity:
                ...
                h_val_no_handling = [u_num[dt][False][False]['state_function'] for dt in dt_list]
                ...
                t_no_handling = [u_num[dt][False][False]['t'] for dt in dt_list]
                ...
                if not use_A and not use_SE:
                    ...
                    h_val = [u_num[dt][use_SE][use_A]['state_function'] for dt in dt_list]
                    h = [item[:] if n == 1 else item[:, i] for item in h_val]
                    h = [item.reshape((item.shape[0],)) for item in h]

                    t = [u_num[dt][use_SE][use_A]['t'] for dt in dt_list]

                    if use_SE:
                        t_switches = [u_num[dt][use_SE][use_A]['t_switches'] for dt in dt_list]
                ...
                ax[0, 0].set_ylabel(r'Absolute values of h $|h(v_{C_n}(t))|$'.format(i + 1), fontsize=16)
                ax[0, ind].grid(visible=True)
                ax[0, ind].minorticks_off()
                ax[0, ind].legend(frameon=True, fontsize=12, loc='lower left')

        fig.savefig('data/{}_comparison_event{}.png'.format(prob_cls_name, i + 1), dpi=300, bbox_inches='tight')
        plt_helper.plt.close(fig)


def state_function_over_time(u_num, prob_cls_name):  # pragma: no cover
    r"""
    ...
    for dt in dt_list:
        ...
        for use_SE in use_detection:
            for use_A in use_adaptivity:
                t = u_num[dt][use_SE][use_A]['t']
                h_val = u_num[dt][use_SE][use_A]['state_function']
        ...
        fig.savefig('data/{}_state_function_over_time_dt={}.png'.format(prob_cls_name, dt), dpi=300, bbox_inches='tight')
        plt_helper.plt.close(fig)
