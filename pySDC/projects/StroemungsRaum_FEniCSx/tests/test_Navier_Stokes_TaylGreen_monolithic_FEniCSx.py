...
def test_eval_f():
    ...
    # get the exact solution at time t
    wex = dfx.fem.Function(prob.W)
    wex.sub(0).interpolate(
        lambda x: np.vstack(
            (
                -1.0 * np.cos(x[0]) * np.sin(x[1]) * np.exp(-2.0 * nu * t),
                np.sin(x[0]) * np.cos(x[1]) * np.exp(-2.0 * nu * t),
            )
        )
    )
