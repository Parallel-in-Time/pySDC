...
class fenics_heat2D(Problem):
    ...
    def __init__(self, c_nvars=64, t0=0.0, family='CG', order=2, refinements=1, nu=0.1, c=0.0):
        ...
        super(fenics_heat2D, self).__init__(self.V)
...
# noinspection PyUnusedLocal
class fenics_heat2D_mass(fenics_heat2D):
    r"""
    Example implementing the forced two-dimensional heat equation with Dirichlet boundary conditions
...
# noinspection PyUnusedLocal
class fenics_heat2D_mass_timebc(fenics_heat2D_mass):
    r"""
    ...
    def __init__(self, c_nvars=64, t0=0.0, family='CG', order=2, refinements=1, nu=0.1, c=0.0):
        ...
        self.u_D = df.Expression('cos(a*x[0]) * cos(a*x[1]) * cos(t) + c', c=self.c, a=np.pi, t=t0, degree=self.order)
        self.bc = df.DirichletBC(self.V, self.u_D, Boundary)
        self.bc_hom = df.DirichletBC(self.V, df.Constant(0), Boundary)

        # set forcing term as expression
        self.g = df.Expression(
            '-cos(a*x[0]) * cos(a*x[1]) * (sin(t) - 2*b*a*a*cos(t))',
            a=np.pi,
            b=self.nu,
            t=self.t0,
            degree=self.order,
    ...
    def u_exact(self, t):
        ...
        u0 = df.Expression('cos(a*x[0]) * cos(a*x[1]) * cos(t) + c', c=self.c, a=np.pi, t=t, degree=self.order)
        me = self.dtype_u(df.interpolate(u0, self.V), val=self.V)
