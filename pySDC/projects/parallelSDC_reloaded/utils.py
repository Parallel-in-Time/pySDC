...
# Utility class for data regression test
class Data:

    DATA = {}  # cache for data
    DATA_REF = None  # cache for reference data
    DATA_FILE = os.path.join(PATH, '_data.json')
    DATA_REF_FILE = os.path.join(PATH, '_dataRef.json')

    @classmethod
    def storeAndCheck(cls, key, data):
        """
        ...
        """
        cls.DATA[key] = list(data)
        if cls.DATA_REF is None:
            try:
                with open(cls.DATA_REF_FILE, "r") as f:
                    cls.DATA_REF = json.load(f)
            except FileNotFoundError:
                print(f"Warning : no reference data to check key:{key}")
                return

        assert key in cls.DATA_REF, f"key:{key} not in reference data"
    ...
    def writeToJSON(cls):
        ...
        if os.path.isfile(cls.DATA_FILE):
