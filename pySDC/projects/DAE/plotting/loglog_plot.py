...
def plot_convergence():
    ...
    for qd_type in data.keys():
        ...
        for num_nodes, color, shape, style, order \
            in zip(num_nodes_list, color_list, shape_list, style_list, order_list):
            ...
            ax.loglog(
                ...
                data[qd_type][num_nodes]['error'][start_point]*(data[qd_type][num_nodes]['dt'][start_point:]/data[qd_type][num_nodes]['dt'][start_point])**order, 
                color="black", 
                ls=style, 
                lw=0.7, 
            ...
            for niter, error, dt, position, offset in \
                zip(data[qd_type][num_nodes]['niter'], 
            ...
            ylim=((min(np.append(data[qd_type][num_nodes]['error'][data_start_point:], ylim[0])),
