...
class exponential_multirate_stabilized(sweeper):
    """
    ...
    def __init__(self, params):
        ...
        super(exponential_multirate_stabilized, self).__init__(params)
    ...
    def update_nodes(self):
        ...
        for m in range(M):
            ...
            f1 = P.eval_f(integral[m]+self.djm1, L.time + L.dt * (self.coll.nodes[m]-self.coll.delta_m[m]),eval_impl=True,eval_expl=False,eval_exp=True)
            f2 = P.eval_f(u_old[m], L.time + L.dt * (self.coll.nodes[m]-self.coll.delta_m[m]),eval_impl=True,eval_expl=False,eval_exp=True)
