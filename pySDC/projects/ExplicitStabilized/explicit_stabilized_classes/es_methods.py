...
class RKC1:
    ...
    def update_coefficients(self, s):
        ...
        c = c[1:]
        c[-1]=1. # correct eventual roundoff error
...
class RKU1:
    ...
    def update_coefficients(self, s):
        ...
        w0 = 1.+3.0*self.damping/(s*(s+1.)*(s+2.))
        w1 = np.polynomial.chebyshev.chebval(w0,c)/np.polynomial.chebyshev.chebval(w0,dc)
...
class HSRKU1:
    ...
    def update_coefficients(self, s):
        ...
        self.c = np.append(self.rku.c,1.+self.rku.c)/2
    ...
    def get_m(self,dt,rho_F,s):
        ...
        if not self.scale_separation:         
            if isinstance(self.inner_method,RKC1):               
                ...
                m = int(np.ceil(np.sqrt( 1.+6.*z/(betai*ls) )))            
                m = max([m,2])
                if self.even_m:
                    m += m%2
