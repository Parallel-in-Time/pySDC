...
class rho_estimator:
    ...
    def rho(self,y,t,fy=None):
        ...
        if self.imex:
            if fy is not None:               
                self.eigval.expl, self.eigvec.expl, self.n_f_eval.expl = self.rho_f(lambda x: self.P.eval_f(x,t,eval_impl=False,eval_expl=True,eval_exp=False).expl,y,fy.expl,self.eigval.expl,self.eigvec.expl,self.n_f_eval.expl)
                self.eigval.impl, self.eigvec.impl, self.n_f_eval.impl = self.rho_f(lambda x: self.P.eval_f(x,t,eval_impl=True,eval_expl=False,eval_exp=False).impl,y,fy.impl,self.eigval.impl,self.eigvec.impl,self.n_f_eval.impl)
            else:                
                self.eigval.expl, self.eigvec.expl, self.n_f_eval.expl = self.rho_f(lambda x: self.P.eval_f(x,t,eval_impl=False,eval_expl=True,eval_exp=False).expl,y,None,self.eigval.expl,self.eigvec.expl,self.n_f_eval.expl)
                self.eigval.impl, self.eigvec.impl, self.n_f_eval.impl = self.rho_f(lambda x: self.P.eval_f(x,t,eval_impl=True,eval_expl=False,eval_exp=False).impl,y,None,self.eigval.impl,self.eigvec.impl,self.n_f_eval.impl)
        else:
            if fy is not None:
