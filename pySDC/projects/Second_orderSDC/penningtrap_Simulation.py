...
class Convergence(object):
    ...
    def plot_convergence(self):
        ...
        for ii in range(0, N):
            ...
            ax1.loglog(time_data[ii,:], error_data['pos'][value, ii, :], ' ', color=color[ii], marker=shape[ii], markersize=fs-2, label='k={}'.format(int(self.K_iter[ii])))
            if value==2:
                ax1.text(time_data[ii,1], 0.3*convline['pos'][value,ii, 1],r"$\mathcal{O}(\Delta t^{%d})$" % (order_data['pos'][ii,0,1]),size=fs+2,)
            else:
                ax1.text(time_data[ii,1], 0.3*convline['pos'][value, ii,1],r"$\mathcal{O}(\Delta t^{%d})$" % (order_data['pos'][ii,0, 0]),size=fs+2,)
        ...
        for ii in range(0, N):
            ...
            ax2.loglog(time_data[ii,:], error_data['vel'][value, ii,:], ' ', color=color[ii], marker=shape[ii], markersize=fs-2, label='k={}'.format(int(self.K_iter[ii])) )

            if value==2:
                ax2.text(time_data[ii,1], 0.3*convline['vel'][value, ii,1],r"$\mathcal{O}(\Delta t^{%d})$" % (order_data['vel'][ii, 0,1]),size=fs+2)
            else:
                ax2.text(time_data[ii,1], 0.3*convline['vel'][value, ii,1],r"$\mathcal{O}(\Delta t^{%d})$" % (order_data['vel'][ii, 0,0]),size=fs+2)
    ...
    def compute_local_error_data(self):
        ...
        for order in self.K_iter:
            ...
            for ii in range(0, self.time_iter):
                ...
                file.write(str(dt_omega) + " * " + str(order_pos) + " * " +str(error['pos'][:,ii]) + " * " + str(order_vel) + " * " + str(error['vel'][:,ii]) + '\n')
    ...
    def find_approximate_order(self, filename='data/Local-conv-data.txt'):
        ...
        for jj in range(0,3):
            ...
            if jj==2:
                file.write(str(order_data['pos'][:, jj, 1]) + ' * ' + str(approx_order['pos'][0])+ ' * ' + str(order_data['vel'][:,jj, 1]) +' * ' +str(approx_order['vel'][0]) + '\n')
            else:

                file.write(str(order_data['pos'][:, jj, 0]) + ' * ' + str(approx_order['pos'][0])+ ' * ' + str(order_data['vel'][:,jj, 0]) +' * ' +str(approx_order['vel'][0]) + '\n')
        file.close()
...
class Work_precision(Convergence):
    ...
    def __init__(self,controller_params, description, time_iter=3, K_iter=[1, 2, 3],Tend=2, axes=[0], RKN=True, VV=True ,cwd=''):
        self.RKN=RKN
        self.VV=VV

        super().__init__(controller_params, description, time_iter=time_iter, K_iter=K_iter,  Tend=Tend, axes=axes, cwd=cwd)
    ...
    def func_eval_SDC(self):
        ...
        for order in self.K_iter:
            ...
            if order ==self.K_iter[0]:
                file=open(self.cwd +'data/func_eval_vs_error_SDC{}{}.txt'.format(self.time_iter, self.num_nodes), 'w')
            else:
                file= open(self.cwd + 'data/func_eval_vs_error_SDC{}{}.txt'.format(self.time_iter, self.num_nodes), 'a')
            ...
            for ii in range(0, time_iter):
                ...
                file.write(str(func_eval) + " * " + str(global_order) + " * " +str(error['position'][:,ii]) + " * " + str(global_order) + " * " + str(error['velocity'][:,ii]) + '\n')
            file.close()
    ...
    def func_eval_Picard(self):
        ...
        for order in self.K_iter:
            ...
            if order ==self.K_iter[0]:
                file=open(self.cwd +'data/func_eval_vs_error_picard{}{}.txt'.format(self.time_iter, self.num_nodes), 'w')
            else:
                file= open(self.cwd + 'data/func_eval_vs_error_picard{}{}.txt'.format(self.time_iter, self.num_nodes), 'a')
            ...
            for ii in range(0, self.time_iter):
                ...
                # instantiate the controller (no controller parameters used here)
                controller = controller_nonMPI(num_procs=1, controller_params=self.controller_params, description=description)
    ...
    def plot_work_precision(self):
        ...
        fs=16
        [N, func_eval_SDC, error_SDC, *_]=self.organize_data(filename=self.cwd + 'data/func_eval_vs_error_SDC{}{}.txt'.format(self.time_iter, self.num_nodes), time_iter=self.time_iter)

        [N, func_eval_picard, error_picard, *_]=self.organize_data(filename=self.cwd + 'data/func_eval_vs_error_picard{}{}.txt'.format(self.time_iter, self.num_nodes), time_iter=self.time_iter)
        ...
        if self.RKN:
            [N, func_eval_RKN, error_RKN, *_]=self.organize_data(filename=self.cwd + 'data/func_eval_vs_error_RKN{}{}.txt'.format(self.time_iter, self.num_nodes), time_iter=self.time_iter)

            ax1.loglog(func_eval_RKN[0][1:], error_RKN['pos'][value,][0][:][1:], ls='dashdot', color='purple', marker='p', label='RKN-4', markersize=fs-3)
            ax2.loglog(func_eval_RKN[0][1:], error_RKN['vel'][value, ][0][:][1:], ls='dashdot', color='purple', marker='p', label='RKN-4', markersize=fs-3)
        if self.VV:

            [N, func_eval_VV, error_VV, *_]=self.organize_data(filename=self.cwd + 'data/func_eval_vs_error_VV{}{}.txt'.format(self.time_iter, self.num_nodes), time_iter=self.time_iter)

            ax1.loglog(func_eval_VV[0], error_VV['pos'][value,][0][:], ls='dashdot', color='blue', marker='H', label='Velocity-Verlet', markersize=fs-3)
            ax2.loglog(func_eval_VV[0], error_VV['vel'][value, ][0][:], ls='dashdot', color='blue', marker='H', label='Velocity-Verlet', markersize=fs-3)
        ...
        for ii, jj in enumerate(self.K_iter):
            ...
            ax1.loglog(func_eval_SDC[ii, :], error_SDC['pos'][value, ii, :], ls='solid', color=color[ii], marker=shape[ii], label='k={}'.format(jj), markersize=fs-2)
            ax1.loglog(func_eval_picard[ii,:], error_picard['pos'][value, ii, :], ls='--', color=color[ii], marker=shape[ii], markersize=fs-2)


            ax2.loglog(func_eval_SDC[ii, :], error_SDC['vel'][value, ii, :], ls='solid', color=color[ii], marker=shape[ii], label='k={}'.format(jj), markersize=fs-2)
            ax2.loglog(func_eval_picard[ii,:], error_picard['vel'][value, ii, :], ls='--', color=color[ii], marker=shape[ii], markersize=fs-2)
...
class Stability_implementation(object):
    ...
    def stability_data(self):
        ...
        for i in range(0, self.kappa_iter):
            for j in range(0, self.mu_iter):
                ...
                if self.K_iter != 0:
                    lambdas = [k, mu]
                    SDC_mat_sweep, Ksdc_eigval = L.sweep.get_scalar_problems_manysweep_mats(nsweeps=self.K_iter, lambdas=lambdas)
    ...
    def plot_stability(self, region, title=""):
        ...
        CS1 = plt.contour(self.lambda_kappa, self.lambda_mu, np.absolute(region.T), levels, colors='k', linestyles="dashed")
