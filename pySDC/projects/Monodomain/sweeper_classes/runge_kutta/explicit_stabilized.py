...
from pySDC.projects.ExplicitStabilized.explicit_stabilized_classes.es_methods import RKC1, RKU1, RKW1
...
class explicit_stabilized(sweeper):
    ...
    def init_es(self, params):
        return [eval(params["es_class"])(self.damping, self.safe_add) for _ in range(self.coll.num_nodes)]
    ...
    def update_stages_coefficients(self):
        ...
        if self.rho_count % self.params.rho_freq == 0:
            ...
            for m in range(M):
                ...
                self.s[m] = self.es[m].get_s(L.dt * self.delta[m] * rho_m)
                if self.s[m] != self.s_prev[m]:
                    self.es[m].update_coefficients(self.s[m])
                    self.sub_nodes[m] = self.coll.nodes[m] + (self.es[m].c - 1.0) * self.delta[m]
    ...
    def update_nodes(self):
        ...
        for m in range(M):
            ...
            f1 = P.eval_f(integral[m] + self.djm1, L.time + L.dt * (self.coll.nodes[m] - self.delta[m]))
            f2 = P.eval_f(u_old[m], L.time + L.dt * (self.coll.nodes[m] - self.delta[m]))
