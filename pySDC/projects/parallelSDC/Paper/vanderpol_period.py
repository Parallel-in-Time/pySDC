...
for mu in muVals:
    ...
    uExact = np.array([solVanderpolExact(t, mu=mu) for t in tVals])
...
for mu, tEnd in zip(muVals, muPeriods):
    ...
    plt.figure(f"{script}_accel_scaled")
    plt.plot(tVals/tEnd, uExact[:, 1], '-', label=f"$\mu=${mu}")
    print(' -- done')

# Figure settings
for figName in [
        f"{script}_traj", f"{script}_accel",
        f"{script}_traj_scaled", f"{script}_accel_scaled"
        ]:
    plt.figure(figName)
    plt.legend()
