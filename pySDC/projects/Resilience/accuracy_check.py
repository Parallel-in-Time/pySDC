...
def multiple_runs(ax, k=5, serial=True):
    ...
    stats, _, _ = run_piline(custom_description=desc, num_procs=num_procs, Tend=30 * dt_list[0], hook_class=log_errors)
    res = get_results_from_stats(stats, 'dt', dt_list[0])
    for key in res.keys():
        res[key] = [res[key]]
    ...
    for i in range(1, len(dt_list)):
        ...
        stats, _, _ = run_piline(custom_description=desc, num_procs=num_procs, Tend=30 * dt_list[i],
