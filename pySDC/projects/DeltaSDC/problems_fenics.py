...
.. warning::
   ...
   **This module has not been executed.** DOLFIN was unavailable in the environment it was written
   in, so the code below is verified only by inspection of the weak forms. The tests are marked
   ``fenics`` and run in CI; treat the module as unvalidated until they have passed there.
...
the arithmetic stays at the backend type.
"""
