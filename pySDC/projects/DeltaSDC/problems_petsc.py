...
class petsc_fisher_delta(petsc_fisher_fullyimplicit):
    ...
    **kwargs
    ...
    def eval_f_increment(self, base, delta, t):
        ...
        Same expansion as :meth:`FisherCorrectionResidual.formFunction` uses inside the solve, but
        at backend precision and without the implicit prefactor: this one is for the sweeper, whose
        accumulation lives on the level. Diffusion is linear, so its increment is the operator
        applied to :math:`\delta`; the reaction term is expanded binomially.
        ...
        self.A.mult(delta, f)
        out = self.init.getVecArray(f)
        w = self.init.getVecArray(base)
        d = self.init.getVecArray(delta)
        for i in range(self.xs, self.xe):
            out[i] += self.lambda0**2 * (d[i] - binomial_increment(w[i], d[i], self.nu + 1))
        out[0] = 0
        out[-1] = 0
        return f
