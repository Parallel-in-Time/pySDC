...
lambda_k, lambda_mu, sweeper_params['num_nodes'], K,  dt*stab.real, dt*Kstab.real, dt*stab_picard.real, dt*KPstab.real        
    ...
    q_mat=np.block([[dt**2*qQ, np.zeros(nnodes)], [np.zeros(nnodes), dt*L.sweep.coll.weights]])
        ...
        for j in range(0,N_mu):
            ...
            if K!=0:
                ...
                if L.sweep.params.picard_mats_sweep:
                    Picard_mats_sweep, Kpicard=L.sweep.get_scalar_problems_picardsweep_mats(nsweeps=K, lambdas=lambdas)
                else:
                    pass
    ...
    return lambda_k, lambda_mu, sweeper_params['num_nodes'], K,  dt*stab.real, dt*Kstab.real, dt*stab_picard.real, dt*KPstab.real
...
def main():
    lambda_k, lambda_mu, num_nodes, K, stab, Kstab, stab_picard, KPstab = compute_stability()
    
    plot_stability(lambda_k, lambda_mu, num_nodes, K, stab, 'SDC stability')
    plot_stability(lambda_k, lambda_mu, num_nodes, K, Kstab, r'$K_{sdc}$ matrix eigenvalue')
    plot_stability(lambda_k, lambda_mu, num_nodes, K, stab_picard, 'Picard stability')
    plot_stability(lambda_k, lambda_mu, num_nodes, K, KPstab, 'Picard iteration matrix eigenvalue')
