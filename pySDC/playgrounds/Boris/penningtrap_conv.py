...
def plot_convergence(cwd=''):
    ...
    for ii, jj in enumerate(Kiter):
        plt.loglog(time*omega_B, error[jj][plot][axis,:], color=color[ii], marker=shape[ii], ls='', ms=fs, label='k={}'.format(jj))
        plt.loglog(time*omega_B, error[jj][plot][axis,0]*(time/time[0])**(order[ii]),color='black')
        plt.text(time[1]*omega_B, 0.3*error[jj][plot][axis, 0]*(time[1]/time[0])**(order[ii]), r'$\mathcal{O}(\Delta t^{%d})$'%(order[ii]), size=2*fs)
    plt.grid(True)
    plt.title(f'$v_{axis+1}$ coordinate, M={num_nodes}')
