...
def evaluate(solution):
    ...
    for i in range(-8, 8):
        for l in range(-8, 8):
            k += 1
            lamdt = -10 ** i + 1j * 10 ** l
            R = np.linalg.inv(np.eye(nsteps * m) - lamdt * np.kron(np.eye(nsteps), Qd) - np.kron(Ea, N)).dot(lamdt * np.kron(np.eye(nsteps), Q - Qd) + np.kron(E - Ea, N))
...
problem = {'problem_name': 'Qdelta_sum_diag',
           'parameters': params,
