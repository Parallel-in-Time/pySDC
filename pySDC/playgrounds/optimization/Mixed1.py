...
def evaluate(solution):
    ...
    obj_val = 0
    if (x['x1'] == "A"):     obj_val = 0.2 + pow(x['x2'] - 0.3456789, 2.0);
    if (x['x1'] == "B"):     obj_val = 0.1 + pow(x['x2'] - 0.3456789, 2.0) + pow(x['x3'] - 0.3456789, 2.0);
    if (x['x1'] == "C"):     obj_val = pow(x['x2'] - 0.456789, 2.0) + pow(x['x3'] - x['x2'] - 0.234567, 2.0);
...
problem = {'problem_name': 'Mixed search space',
            'parameters': params,
