...
def run_newton_pfasst(Tend=None):
    ...
    controller_prec = allinclusive_multigrid_nonMPI(num_procs=num_procs, controller_params=controller_params,
                                                      description=description)
    controller_prec.params.predict = True
    for S in controller_prec.MS:
        S.params.maxiter = 0

    # get initial values on finest level
    P = controller_prec.MS[0].levels[0].prob
    uinit = P.u_exact(t0)
    uend, stats = controller_prec.run(u0=uinit, t0=t0, Tend=Tend)

    uk = np.concatenate([S.levels[0].u[m].values for S in controller_prec.MS for m in range(1, description['sweeper_params']['num_nodes'] + 1)])
