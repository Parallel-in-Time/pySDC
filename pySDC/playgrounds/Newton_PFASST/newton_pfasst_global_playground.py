...
def run_newton_pfasst(Tend=None):
    ...
    while np.linalg.norm(controller.rhs, np.inf) > 1E-08 or k == 0:
        norm_old = np.linalg.norm(controller.rhs, np.inf)
        k += 1
        ek, stats = controller.run(uk=uk, t0=t0, Tend=Tend)
        u_tmp = uk - ek
        controller.compute_rhs(u_tmp, t0)
        norm_new = np.linalg.norm(controller.rhs, np.inf)
        j = 0
        while not norm_new < (1 - 0.25 * 0.5 ** j) * norm_old:
            j += 1
            u_tmp = uk - 0.5 ** j * ek
            controller.compute_rhs(u_tmp, t0)
            norm_new = np.linalg.norm(controller.rhs, np.inf)
