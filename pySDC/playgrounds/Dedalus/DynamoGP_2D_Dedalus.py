...
class dynamogp_2d_dedalus(ptype):
    ...
    def eval_f(self, u, t):
        ...
        by_y = u.values[1].differentiate(y=1)
        by_z = u.values[1].differentiate(z=1)
        bz_y = u.values[0].differentiate(y=1)
        bz_z = u.values[0].differentiate(z=1)

        f.expl.values[1] = (-self.u * u.values[1] * 1j * self.params.kx - self.v * by_y - self.w * by_z +
                            u.values[0] * self.v_z).evaluate()
        f.expl.values[0] = (-self.u * u.values[0] * 1j * self.params.kx - self.v * bz_y - self.w * bz_z +
                            u.values[1] * self.w_y).evaluate()
    ...
    def solve_system(self, rhs, factor, u0, t):
        ...
        self.bz['g'] = rhs.values[0]['g']
        self.bz['c'] = rhs.values[0]['c']
        self.by['g'] = rhs.values[1]['g']
        self.by['c'] = rhs.values[1]['c']

        self.solver.step(factor)

        me = self.dtype_u(self.init)
        me.values[0]['g'] = self.bz['g']
        me.values[0]['c'] = self.bz['c']
        me.values[1]['g'] = self.by['g']
        me.values[1]['c'] = self.by['c']
